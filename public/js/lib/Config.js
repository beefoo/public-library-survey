const Config = {
  filterBy: [
    {
      label: 'Locale type',
      field: 'locale_type',
      type: 'range',
      values: [
        { minValue: 0, maxValue: 19, label: 'City' },
        { minValue: 20, maxValue: 29, label: 'Suburb' },
        { minValue: 30, maxValue: 50, label: 'Town or rural' },
      ],
    },
    {
      label: 'State',
      field: 'state',
      type: 'value',
      values: 'auto',
    },
    {
      label: 'Region',
      field: 'region',
      type: 'value',
      values: [
        { value: 1, label: 'New England' },
        { value: 2, label: 'Mid East' },
        { value: 3, label: 'Great Lakes' },
        { value: 4, label: 'Plains' },
        { value: 5, label: 'Southeast' },
        { value: 6, label: 'Southwest' },
        { value: 7, label: 'Rocky Mountains' },
        { value: 8, label: 'Far West' },
        { value: 9, label: 'Outlying Areas' },
      ],
    },
    {
      label: 'Operating budget',
      field: 'op_revenue',
      type: 'range',
      values: [
        { minValue: 30000000, label: '$30M+' },
        { minValue: 10000000, maxValue: 30000000, label: '$10M - $30M' },
        { minValue: 5000000, maxValue: 10000000, label: '$5M - $10M' },
        { minValue: 1000000, maxValue: 5000000, label: '$1M - $5M' },
        { minValue: 400000, maxValue: 1000000, label: '$400K - $1M' },
        { minValue: 200000, maxValue: 400000, label: '$200K - $400K' },
        { minValue: 100000, maxValue: 200000, label: '$100K - $200K' },
        { minValue: 50000, maxValue: 100000, label: '$50K - $100K' },
        { minValue: 0, maxValue: 50000, label: 'Below $50K' },
      ],
    },
    {
      label: 'Operating budget per capita',
      field: 'op_revenue_per_capita',
      type: 'range',
      values: [
        { minValue: 500, label: '$500+' },
        { minValue: 250, maxValue: 500, label: '$250 - $500' },
        { minValue: 100, maxValue: 250, label: '$100 - $250' },
        { minValue: 75, maxValue: 100, label: '$75 - $100' },
        { minValue: 50, maxValue: 75, label: '$50 - $75' },
        { minValue: 25, maxValue: 50, label: '$25 - $50' },
        { minValue: 10, maxValue: 25, label: '$10 - $25' },
        { minValue: 0, maxValue: 10, label: 'Under $10' },
      ],
    },
    {
      label: 'Staff size',
      field: 'staff',
      type: 'range',
      values: [
        { minValue: 100, label: '100+' },
        { minValue: 25, maxValue: 100, label: '25 - 100' },
        { minValue: 10, maxValue: 25, label: '10 - 25' },
        { minValue: 0, maxValue: 10, label: 'Under 10' },
      ],
    },
    {
      label: 'Visits per capita',
      field: 'visits_per_capita_norm',
      type: 'perc_range',
      values: [
        { minValue: 80, label: 'Highest 20%' },
        { minValue: 60, maxValue: 80, label: 'Higher 20%' },
        { minValue: 40, maxValue: 60, label: 'Middle 20%' },
        { minValue: 20, maxValue: 40, label: 'Lower 20%' },
        { minValue: 0, maxValue: 20, label: 'Lowest 20%' },
      ],
    },
    {
      label: 'Programs per capita',
      field: 'programs_per_capita_norm',
      type: 'perc_range',
      values: [
        { minValue: 80, label: 'Highest 20%' },
        { minValue: 60, maxValue: 80, label: 'Higher 20%' },
        { minValue: 40, maxValue: 60, label: 'Middle 20%' },
        { minValue: 20, maxValue: 40, label: 'Lower 20%' },
        { minValue: 0, maxValue: 20, label: 'Lowest 20%' },
      ],
    },
    {
      label: 'Attendance per program',
      field: 'attendance_per_program_norm',
      type: 'perc_range',
      values: [
        { minValue: 80, label: 'Highest 20%' },
        { minValue: 60, maxValue: 80, label: 'Higher 20%' },
        { minValue: 40, maxValue: 60, label: 'Middle 20%' },
        { minValue: 20, maxValue: 40, label: 'Lower 20%' },
        { minValue: 0, maxValue: 20, label: 'Lowest 20%' },
      ],
    },
    {
      label: 'Computer sessions per capita',
      field: 'computer_per_capita_norm',
      type: 'perc_range',
      values: [
        { minValue: 80, label: 'Highest 20%' },
        { minValue: 60, maxValue: 80, label: 'Higher 20%' },
        { minValue: 40, maxValue: 60, label: 'Middle 20%' },
        { minValue: 20, maxValue: 40, label: 'Lower 20%' },
        { minValue: 0, maxValue: 20, label: 'Lowest 20%' },
      ],
    },
    {
      label: 'Wifi sessions per capita',
      field: 'wifi_per_capita_norm',
      type: 'perc_range',
      values: [
        { minValue: 80, label: 'Highest 20%' },
        { minValue: 60, maxValue: 80, label: 'Higher 20%' },
        { minValue: 40, maxValue: 60, label: 'Middle 20%' },
        { minValue: 20, maxValue: 40, label: 'Lower 20%' },
        { minValue: 0, maxValue: 20, label: 'Lowest 20%' },
      ],
    },
    {
      label: 'Median household income (Census)',
      field: 'income',
      type: 'range',
      values: [
        { minValue: 200000, label: '$200K+' },
        { minValue: 150000, maxValue: 200000, label: '$150K - $200K' },
        { minValue: 100000, maxValue: 150000, label: '$100K - $150K' },
        { minValue: 75000, maxValue: 100000, label: '$75K - $100K' },
        { minValue: 50000, maxValue: 75000, label: '$50K - $75K' },
        { minValue: 25000, maxValue: 50000, label: '$25K - $50K' },
        { minValue: 0, maxValue: 25000, label: 'Under $25K' },
      ],
    },
    {
      label: 'Percent White (Census)',
      field: 'perc_white',
      type: 'perc_range',
      values: [
        { minValue: 67, label: 'Over 66%' },
        { minValue: 33, maxValue: 67, label: '33% to 66%' },
        { minValue: 10, maxValue: 33, label: '10% to 33%' },
        { minValue: 5, maxValue: 10, label: '5% to 10%' },
        { minValue: 0, maxValue: 5, label: 'Less than 5%' },
      ],
    },
    {
      label: 'Percent Black (Census)',
      field: 'perc_black',
      type: 'perc_range',
      values: [
        { minValue: 67, label: 'Over 66%' },
        { minValue: 33, maxValue: 67, label: '33% to 66%' },
        { minValue: 10, maxValue: 33, label: '10% to 33%' },
        { minValue: 5, maxValue: 10, label: '5% to 10%' },
        { minValue: 0, maxValue: 5, label: 'Less than 5%' },
      ],
    },
    {
      label: 'Percent Hispanic/Latino (Census)',
      field: 'perc_hispanic',
      type: 'perc_range',
      values: [
        { minValue: 67, label: 'Over 66%' },
        { minValue: 33, maxValue: 67, label: '33% to 66%' },
        { minValue: 10, maxValue: 33, label: '10% to 33%' },
        { minValue: 5, maxValue: 10, label: '5% to 10%' },
        { minValue: 0, maxValue: 5, label: 'Less than 5%' },
      ],
    },
    {
      label: 'Percent Indigenous (Census)',
      field: 'perc_indigenous',
      type: 'perc_range',
      values: [
        { minValue: 67, label: 'Over 66%' },
        { minValue: 33, maxValue: 67, label: '33% to 66%' },
        { minValue: 10, maxValue: 33, label: '10% to 33%' },
        { minValue: 5, maxValue: 10, label: '5% to 10%' },
        { minValue: 0, maxValue: 5, label: 'Less than 5%' },
      ],
    },
    {
      label: 'Percent Asian/Pacific Islander (Census)',
      field: 'perc_api',
      type: 'perc_range',
      values: [
        { minValue: 67, label: 'Over 66%' },
        { minValue: 33, maxValue: 67, label: '33% to 66%' },
        { minValue: 10, maxValue: 33, label: '10% to 33%' },
        { minValue: 5, maxValue: 10, label: '5% to 10%' },
        { minValue: 0, maxValue: 5, label: 'Less than 5%' },
      ],
    },
    {
      label: 'Percent Under 18 (Census)',
      field: 'perc_minor',
      type: 'perc_range',
      values: [
        { minValue: 50, label: 'Over 50%' },
        { minValue: 40, maxValue: 50, label: '40% to 50%' },
        { minValue: 30, maxValue: 40, label: '30% to 40%' },
        { minValue: 20, maxValue: 30, label: '20% to 30%' },
        { minValue: 0, maxValue: 20, label: 'Less than 20%' },
      ],
    },
    {
      label: 'Percent 65 and over (Census)',
      field: 'perc_senior',
      type: 'perc_range',
      values: [
        { minValue: 30, label: 'Over 30%' },
        { minValue: 20, maxValue: 30, label: '20% to 30%' },
        { minValue: 10, maxValue: 20, label: '10% to 20%' },
        { minValue: 0, maxValue: 10, label: 'Less than 10%' },
      ],
    },
    {
      label: '2020 Presidential Election (MIT)',
      field: 'vote_points',
      type: 'range',
      values: [
        { minValue: 20, maxValue: 100, label: 'Solid Republican' },
        { minValue: 5, maxValue: 20, label: 'Lean Republican' },
        { minValue: -5, maxValue: 5, label: 'Toss up' },
        { minValue: -20, maxValue: -5, label: 'Lean Democrat' },
        { minValue: -100, maxValue: -20, label: 'Solid Democrat' },
      ],
    },
  ],

  sortBy: [
    {
      field: 'name-asc',
      label: 'Name (A-Z)',
      isalpha: 1,
    },
    {
      field: 'pop_lsa-desc',
      label: 'Number of people served (high to low)',
      direction: 'desc',
    },
    {
      field: 'pop_lsa-asc',
      label: 'Number of people served (low to high)',
    },
    {
      field: 'visits_per_capita_norm-desc',
      label: 'Visits per capita (high to low)',
    },
    {
      field: 'programs_per_capita_norm-desc',
      label: 'Programs per capita (high to low)',
    },
    {
      field: 'attendance_per_program_norm-desc',
      label: 'Attendance per program (high to low)',
    },
    {
      field: 'computer_per_capita_norm-desc',
      label: 'Computer sessions per capita (high to low)',
    },
    {
      field: 'wifi_per_capita_norm-desc',
      label: 'Wifi sessions per capita (high to low)',
    },
    {
      field: 'op_revenue-desc',
      label: 'Operating revenue (high to low)',
    },
    {
      field: 'op_revenue-asc',
      label: 'Operating revenue (low to high)',
    },
    {
      field: 'op_revenue_per_capita-desc',
      label: 'Op revenue per capita (high to low)',
    },
    {
      field: 'op_revenue_per_capita-asc',
      label: 'Op revenue per capita (low to high)',
    },
    {
      field: 'staff-desc',
      label: 'Staff size (high to low)',
    },
    {
      field: 'staff-asc',
      label: 'Staff size (low to high)',
    },
    {
      field: 'perc_white-desc',
      label: 'Percent White (high to low)',
    },
    {
      field: 'perc_white-asc',
      label: 'Percent White (low to high)',
    },
    {
      field: 'perc_black-desc',
      label: 'Percent Black (high to low)',
    },
    {
      field: 'perc_hispanic-desc',
      label: 'Percent Hispanic/Latino (high to low)',
    },
    {
      field: 'perc_indigenous-desc',
      label: 'Percent Indigenous (high to low)',
    },
    {
      field: 'perc_api-desc',
      label: 'Percent Asian/Pacific Islander (high to low)',
    },
    {
      field: 'median_age-asc',
      label: 'Median age (younger to older)',
    },
    ,
    {
      field: 'median_age-desc',
      label: 'Median age (older to younger)',
    },
    {
      field: 'income-desc',
      label: 'Median household income (low to high)',
    },
  ],

  colorBy: [
    {
      label: 'Number of people served',
      field: 'pop_lsa',
      type: 'quant',
      minValue: 0,
      pow: 0.33,
    },
    {
      label: 'Visits per capita',
      field: 'visits_per_capita_norm',
      type: 'quant',
      minValue: 0,
    },
    {
      label: 'Operating Revenue',
      field: 'op_revenue',
      type: 'quant',
      minValue: 0,
      pow: 0.3,
    },
    {
      label: 'Operating Revenue per capita',
      field: 'op_revenue_per_capita',
      type: 'quant',
      minValue: 0,
      pow: 0.3,
    },
    {
      label: 'Programs per capita',
      field: 'programs_per_capita_norm',
      type: 'quant',
      minValue: 0,
    },
    {
      label: 'Attendance per program',
      field: 'attendance_per_program_norm',
      type: 'quant',
      minValue: 0,
    },
    {
      label: 'Computer sessions per capita',
      field: 'computer_per_capita_norm',
      type: 'quant',
      minValue: 0,
    },
    {
      label: 'Wifi sessions per capita',
      field: 'wifi_per_capita_norm',
      type: 'quant',
      minValue: 0,
    },
    {
      label: 'Staff size',
      field: 'staff',
      type: 'quant',
      minValue: 0,
    },
    {
      label: 'Median Household Income <small><em>(Census)</em></small>',
      field: 'income',
      type: 'quant',
      minValue: 0,
    },
    {
      label: 'Percent White <small><em>(Census)</em></small>',
      field: 'perc_white',
      type: 'quant',
      minValue: 0,
      maxValue: 100,
    },
    {
      label: 'Percent Black <small><em>(Census)</em></small>',
      field: 'perc_black',
      type: 'quant',
      minValue: 0,
      maxValue: 100,
    },
    {
      label: 'Percent Hispanic or Latino <small><em>(Census)</em></small>',
      field: 'perc_hispanic',
      type: 'quant',
      minValue: 0,
      maxValue: 100,
    },
    {
      label: 'Percent Indigenous <small><em>(Census)</em></small>',
      field: 'perc_indigenous',
      type: 'quant',
      minValue: 0,
      maxValue: 100,
    },
    {
      label: 'Percent Asian/Pacific Islander <small><em>(Census)</em></small>',
      field: 'perc_api',
      type: 'quant',
      minValue: 0,
      maxValue: 100,
    },
    {
      label: 'Median age <small><em>(Census)</em></small>',
      field: 'median_age',
      type: 'quant',
      minValue: 0,
      maxValue: 100,
      labelLeft: 'Younger',
      labelRight: 'Older',
    },
    {
      label: '2020 Presidential Election <small><em>(MIT)</em></small>',
      field: 'vote_points',
      type: 'quant',
      minValue: -100,
      maxValue: 100,
      labelLeft: 'Dem',
      labelRight: 'Rep',
    },
  ],

  localeTypes: [
    { value: 11, label: 'Large city' },
    { value: 12, label: 'Mid-size city' },
    { value: 13, label: 'Small city' },
    { value: 21, label: 'Large suburb' },
    { value: 22, label: 'Mid-size suburb' },
    { value: 23, label: 'Small suburb' },
    { value: 31, label: 'Town' },
    { value: 32, label: 'Distant town' },
    { value: 33, label: 'Remote town' },
    { value: 41, label: 'Rural' },
    { value: 42, label: 'Distant rural' },
    { value: 43, label: 'Remote rural' },
  ],

  regions: [
    { value: 1, label: 'New England' },
    { value: 2, label: 'Mid East' },
    { value: 3, label: 'Great Lakes' },
    { value: 4, label: 'Plains' },
    { value: 5, label: 'Southeast' },
    { value: 6, label: 'Southwest' },
    { value: 7, label: 'Rocky Mountains' },
    { value: 8, label: 'Far West' },
    { value: 9, label: 'Outlying Areas' },
  ],
};

export default Config;
